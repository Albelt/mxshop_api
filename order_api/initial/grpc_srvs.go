...
func InitOrderSrvCli(tracer opentracing.Tracer) {
	// 通过consul获取grpc服务的地址并配置客户端负载均衡策略
	grpcConsulUrl := fmt.Sprintf(grpcConsulUrlFormat, global.Config.Consul.Addr, global.Config.GrpcSrvs.OrderSrvName)
	...
	global.OrderSrvCli = pb.NewOrderServiceClient(conn)
	zap.S().Info("InitOrderSrvCli ok.")
}
