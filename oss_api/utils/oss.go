...
func Get_policy_token(ossCfg *config.Oss) string {
	now := time.Now().Unix()
	...
	condition = append(condition, ossCfg.UploadDir)
	config.Conditions = append(config.Conditions, condition)

	//calucate signature
	result, err := json.Marshal(config)
	debyte := base64.StdEncoding.EncodeToString(result)
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(ossCfg.AccesskeySecret))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = ossCfg.CallbackUrl
	...
	policyToken.AccessKeyId = ossCfg.AccesskeyId
	policyToken.Host = ossCfg.Host
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = ossCfg.UploadDir
